...
const ConseilAdmin = () => {
    ...
    const handleFileChange = (e) => {
        if (modalIsOpen) {
            setEditingConseil({ ...editingConseil, [e.target.name]: e.target.files[0] });
        } else {
    ...
    const handleAddConseil = () => {
        ...
        axios.post('/api/conseils/add', formData, {
            headers: {
    ...
    const handleDeleteConseil = id => {
        axios.delete(`/api/conseils/delete/${id}`)
    ...
    const handleUpdateConseil = () => {
        ...
        axios.put(`/api/conseils/update/${editingConseil.id}`, formData, {
            headers: {
                'Content-Type': 'multipart/form-data',
            },
        })
        .then(response => {
            setConseils(conseils.map(item => item.id === editingConseil.id ? response.data : item));
            closeModal();
        })
        .catch(error => console.log(error));
    };
...
<div className="mb-8">
    <h2 className="text-xl font-semibold mb-3">Ajouter un nouveau conseil</h2>
    <input
        type="text"
        name="lien"
        placeholder="Lien"
 ...
 <input
    ...
    <input
        ...
        className="w-full px-3 py-2 mb-3 text-base text-gray-700 border rounded-lg focus:shadow-outline"
    />
    <button onClick={handleAddConseil} className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">
        Ajouter
    </button>
</div>
...
<div>
    {conseils.map(conseil => (
        <div key={conseil.id} className="border p-4 mb-4 bg-white shadow rounded">
            ...
            {conseil.image && <img src={conseil.image} alt="Image du conseil" className="w-full h-auto rounded-lg mt-2" />}
            {conseil.logoUrl && <img src={conseil.logoUrl} alt="Logo" className="w-full h-auto rounded-lg mt-2" />}
            <div className="flex justify-end mt-2">
                <button onClick={() => openModalWithConseil(conseil)}
                    className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded mr-2">
                    Modifier
                </button>
                <button onClick={() => handleDeleteConseil(conseil.id)}
...
{modalIsOpen && (
    <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full">
        <div className="relative top-20 mx-auto p-5 border w-1/3 shadow-lg rounded-md bg-white">
            ...
            <div>
                <label className="block mb-2 text-sm font-bold text-gray-700">Image Principale:</label>
                <input
                    type="file"
                    name="image"
                    onChange={handleModalFileChange}  // Vous pouvez avoir besoin d'une fonction distincte pour la modification
                    className="w-full px-3 py-2 mb-3 text-base text-gray-700 border rounded-lg focus:shadow-outline"
                />
            </div>
            ...
            <div>
                <label className="block mb-2 text-sm font-bold text-gray-700">Image du Logo:</label>
                <input
                    type="file"
                    name="logoUrl"
                    onChange={handleModalFileChange}  // De même, une fonction distincte pour la modification peut être nécessaire
                    className="w-full px-3 py-2 mb-3 text-base text-gray-700 border rounded-lg focus:shadow-outline"
                />
            </div>
            <div className="flex justify-between">
                <button onClick={handleUpdateConseil}
